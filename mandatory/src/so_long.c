...
/*   Updated: 2025/05/31 14:15:40 by mfernand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 	if (!m->mlx)
// 		return (1);
// 	m->window = mlx_new_window(m->mlx, width_window(m), height_window(m),
// 			"My Game");
// 	if (!m->window)
// 	{
...
int	main(int ac, char **av)
{
	...
	if (!m.mlx)
		return (1);
	m.window = mlx_new_window(m.mlx,(width_window(&m) * TILE),
	(height_window(&m) * TILE), "My Game");
	if (!m.window)
	{
		mlx_destroy_display(m.mlx);
		return (free(m.mlx), 1);
